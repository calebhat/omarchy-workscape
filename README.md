...
**Pinned outputs become real config.** A profile that pins a scale (and mode) for a display is persisted as Hyprland monitor rules in a marked block at the bottom of `~/.config/hypr/monitors.lua` — rewritten on every apply, removed when nothing is pinned, stripped by `--restore-hypr`. That way reloads and Omarchy's internal-panel watcher agree with what WorkScape set instead of reverting it; the rest of the file stays yours.

**Re-apply when the shell restarts** is **off** by default. When on, a few seconds after the shell (re)starts WorkScape runs one forced scan: it settles the display set and re-applies the matching profile even when nothing looks changed, because a restart can disturb runtime state (workspace pins, rules) the fingerprint cannot see. Occupied workspaces are still left alone.
...
- **Scale / Res per display**: each display row has a Scale dropdown (Auto / 1 / 1.25 / 1.5 / 1.75 / 2 / 2.5 / 3, plus the live value when it isn’t a preset) and a Resolution dropdown fed by the display’s own mode list. **Auto** keeps whatever the display runs now; a picked value is set on the display immediately (via `workscape.sh --set-output`, position untouched) and re-applied with the profile on every apply, including a fractional laptop panel scale that Hyprland’s auto rules get wrong. Saved per profile, so the same display can run different settings elsewhere. Saving a profile or capturing the arrangement snapshots the current scale and mode; switch back to **Auto** to unpin. A profile with no saved displays (the Default fallback) lists the connected ones instead, marked *not saved in this profile yet* — picking a scale or resolution (or capturing the arrangement) adopts that display, and the profile then requires it like any other.
